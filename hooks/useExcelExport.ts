...
interface HeaderGroup {
  title: string;
  span: number;
  style?: Record<string, unknown>; // Styl dla komórki grupującej
}

interface StyleCallbacks {
  header?: (row: ExcelRow) => void;
  data?: (row: ExcelRow, index: number) => void;
  total?: (row: ExcelRow) => void;
}
...
interface ExportConfig {
  ...
  data: unknown[];
  dataMapper: (item: unknown, index: number) => Record<string, unknown>;
