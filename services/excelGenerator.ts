...
export const excelGenerator = {
  ...
  generateManagementReport: async ({ firma, wyniki, prowizjaProc }: ReportData) => {
    ...
    kpiValuesRow.getCell(4).font = {
      name: 'Calibri',
      size: 12,
      bold: true,
      color: { argb: 'FF059669' },
    };
    kpiValuesRow.getCell(5).font = {
      name: 'Calibri',
      size: 12,
      bold: true,
      color: { argb: 'FF059669' },
  ...
  generateImportTemplate: async (rowsToGenerate: number = 10) => {
    ...
    const Workbook = window.ExcelJS.Workbook;
    const workbook = new Workbook();
    const worksheet = workbook.addWorksheet('Szablon Import Pracowników');
    ...
    const zusList =
      '"Pełne składki,Pełne składki z dobrowolną chorobową,Bez dobrowolnej chorobowej,Student/uczeń do 26 (bez ZUS),Inny tytuł do ubezpieczeń (tylko zdrowotna),Emeryt/rencista"';
