...
export const generateJakjestpodzielonawygenerowanawartoV3 = (firma: Firma, totals: any, date: string, sector: string) => `
<div class="page">

  ${generatePageHeaderV3('Skąd Różnica? Pełny Rozkład Wartości', '03. Skąd różnica', 3, 11, date)}
<div class="page-body" style="padding:6px 40px 58px">
    ...
    <div style="margin-bottom:8px">
      ...
      <div style="display:grid;grid-template-columns:200px 1fr 100px;gap:0;border-bottom:1px solid var(--border);padding:3px 0;align-items:start">
        <div>
      ...
      <div style="display:grid;grid-template-columns:200px 1fr 100px;gap:0;border-bottom:1px solid var(--border);padding:3px 0;background:rgba(212,175,90,.04);align-items:start">
        ...
        <div style="font-size:9.5px;color:var(--sp-text-muted);padding-right:12px;line-height:1.4">Pracownik dostaje o ${new Intl.NumberFormat('pl-PL',{minimumFractionDigits:2,maximumFractionDigits:2}).format(totals.sumaRaise)} zł więcej w formie świadczenia rzeczowego (voucher EBS) każdego miesiąca — nie jako gotówkę na konto. Finansowane przez Stratton Prime w ramach modelu PLUS — nie obciąża budżetu pracodawcy.</div>
      ...
      <!-- Wiersz 4 — opłata EBS -->
      <div style="display:grid;grid-template-columns:200px 1fr 100px;gap:0;border-bottom:1px solid var(--border);padding:3px 0;align-items:start">
        <div style="padding-left:14px;border-left:3px solid #DC2626">
        ...
        <div style="font-size:9.5px;color:var(--sp-text-muted);padding-right:12px;line-height:1.4">Wynagrodzenie Stratton Prime za technologię, obsługę prawną, wdrożenie, dokumentację i bieżące utrzymanie platformy EBS. Płatna przez pracodawcę. Jedyna pozycja kosztowa po stronie klienta. (Podwyżka pracownika i bonus kadr są finansowane przez Stratton w ramach pełnej prowizji ${new Intl.NumberFormat('pl-PL',{minimumFractionDigits:2,maximumFractionDigits:2}).format(totals.commission)} zł.)</div>
        <div style="text-align:right">
          <span style="font-family:var(--font-serif);font-size:20px;color:#DC2626;font-weight:700">−${new Intl.NumberFormat('pl-PL',{minimumFractionDigits:2,maximumFractionDigits:2}).format(totals.commission - totals.sumaRaise - totals.sumaAdminBonus)} zł</span>
          <div style="font-size:9px;color:var(--sp-text-muted)">opłata za usługę</div>
        </div>
      </div>

      <!-- Wiersz SUMA -->
      <div style="display:grid;grid-template-columns:200px 1fr 100px;gap:0;background:var(--sp-navy);padding:7px 12px;margin-top:1px">
        <div>
          <div style="font-size:12px;font-weight:700;color:var(--sp-gold-light)">Potencjalna oszczędność pracodawcy (po odliczeniu wszystkich kosztów)</div>
          <div style="font-size:10px;color:rgba(255,255,255,.35)">Szacunkowy zysk netto w budżecie firmy</div>
        </div>
        <div style="font-size:9.5px;color:rgba(255,255,255,.55);padding-right:12px;line-height:1.4">Pięć strumieni wartości zsumowanych: dwa oszczędnościowe (+${new Intl.NumberFormat('pl-PL',{minimumFractionDigits:2,maximumFractionDigits:2}).format(totals.efektStruktury)} i +${new Intl.NumberFormat('pl-PL',{minimumFractionDigits:2,maximumFractionDigits:2}).format(totals.zusSaved)} zł) minus trzy wypływy do pracownika, kadr i Stratton Prime. Pracodawca zatrzymuje ponad połowę wygenerowanej wartości.</div>
        <div style="text-align:right">
          <span style="font-family:var(--font-serif);font-size:22px;color:var(--sp-gold-light);font-weight:700">+${new Intl.NumberFormat('pl-PL',{minimumFractionDigits:2,maximumFractionDigits:2}).format(totals.savingsPlus)} zł</span>
          <div style="font-size:9px;color:rgba(255,255,255,.35)">miesięcznie</div>
        </div>
      </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:1px;background:var(--border);margin-bottom:10px">
      <div style="background:#d1fae5;padding:7px 8px;text-align:center;border-top:3px solid var(--success)">
        <div style="font-size:8.5px;font-weight:600;letter-spacing:.12em;text-transform:uppercase;color:var(--success);margin-bottom:6px">Pracodawca</div>
        <div style="font-family:var(--font-serif);font-size:22px;color:var(--success);line-height:1;margin-bottom:4px">+${new Intl.NumberFormat('pl-PL',{minimumFractionDigits:2,maximumFractionDigits:2}).format(totals.savingsPlus)} zł</div>
        <div style="font-size:10.5px;color:var(--sp-text-muted)">Oszczędność netto po wszystkich kosztach</div>
        <div style="margin-top:8px;font-size:10px;font-weight:600;color:var(--success)">oszczędność netto</div>
      </div>
      <div style="background:var(--sp-gray);padding:7px 8px;text-align:center;border-top:3px solid var(--sp-gold)">
        <div style="font-size:8.5px;font-weight:600;letter-spacing:.12em;text-transform:uppercase;color:var(--sp-gold);margin-bottom:4px">Pracownik</div>
        <div style="font-family:var(--font-serif);font-size:22px;color:var(--sp-gold);line-height:1;margin-bottom:4px">${new Intl.NumberFormat('pl-PL',{minimumFractionDigits:2,maximumFractionDigits:2}).format(totals.sumaRaise)} zł</div>
        <div style="font-size:10.5px;color:var(--sp-text-muted)">Wzrost wartości świadczeń rzeczowych co miesiąc</div>
        <div style="margin-top:8px;font-size:10px;font-weight:600;color:var(--sp-gold)">wzrost netto / m-c</div>
      </div>
      <div style="background:var(--sp-gray);padding:7px 8px;text-align:center;border-top:3px solid var(--sp-gold)">
        <div style="font-size:8.5px;font-weight:600;letter-spacing:.12em;text-transform:uppercase;color:var(--sp-gold);margin-bottom:4px">Kadry / Księgowość</div>
        <div style="font-family:var(--font-serif);font-size:22px;color:var(--sp-gold);line-height:1;margin-bottom:4px">${new Intl.NumberFormat('pl-PL',{minimumFractionDigits:2,maximumFractionDigits:2}).format(totals.sumaAdminBonus)} zł</div>
        <div style="font-size:10.5px;color:var(--sp-text-muted)">Bonus administracyjny za obsługę EBS</div>
        <div style="font-size:9px;color:var(--sp-text-muted);margin-top:4px">finansowane przez SP</div>
        <div style="margin-top:6px;font-size:10px;font-weight:600;color:var(--sp-gold)">bonus miesięczny</div>
      </div>
      <div style="background:var(--sp-navy);padding:7px 8px;text-align:center;border-top:3px solid rgba(255,255,255,.2)">
        <div style="font-size:8.5px;font-weight:600;letter-spacing:.12em;text-transform:uppercase;color:rgba(212,175,90,.6);margin-bottom:6px">Stratton Prime (opłata)</div>
        <div style="font-family:var(--font-serif);font-size:22px;color:var(--sp-gold-light);line-height:1;margin-bottom:4px">${new Intl.NumberFormat('pl-PL',{minimumFractionDigits:2,maximumFractionDigits:2}).format(totals.commission - totals.sumaRaise - totals.sumaAdminBonus)} zł</div>
        <div style="font-size:10.5px;color:rgba(255,255,255,.4)">Technologia, prawo, wdrożenie, utrzymanie</div>
        <div style="margin-top:8px;font-size:10px;font-weight:600;color:var(--sp-gold-light)">opłata za usługę</div>
      </div>
    </div>

    <div class="box-gold">
      <div style="font-size:9px;font-weight:700;letter-spacing:.15em;text-transform:uppercase;color:var(--sp-gold);margin-bottom:8px">Wniosek: model jest zaprojektowany tak, żeby wygrały trzy strony</div>
      <p style="font-size:11px;color:var(--sp-text-muted);line-height:1.6">Pracodawca zatrzymuje ponad połowę wygenerowanej wartości jako oszczędność netto. Pracownik dostaje realnie wyższe świadczenia rzeczowe (voucher EBS) każdego miesiąca — bez obciążenia budżetu firmy. Kadry otrzymują bonus za dodatkową pracę administracyjną. Wszystkie trzy strony zyskują realnie — z jednej puli ZUS, która wcześniej trafiała wyłącznie do systemu ubezpieczeń.</p>
    </div>
