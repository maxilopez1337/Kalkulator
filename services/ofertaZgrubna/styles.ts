export const getOfertaZgrubnaStyles = () => `
  @import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&family=Playfair+Display:ital,wght@0,400;0,600;0,700;1,400;1,600&family=JetBrains+Mono:wght@400;500;600&display=swap');
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .toolbar {
    ...
    height: 48px; background: rgba(10,17,40,0.94);
  ...
  .toolbar-logo {
    ...
    color: var(--sp-gold); font-size: 11px; letter-spacing: 0.15em; font-weight: 700; text-transform: uppercase;
  }
  .toolbar-logo .dot { width: 5px; height: 5px; background: var(--sp-gold); border-radius: 50%; display: inline-block; flex-shrink: 0; }
  .toolbar-right { display: flex; align-items: center; gap: 16px; font-size: 11px; color: rgba(255,255,255,0.35); letter-spacing: 0.1em; }

  /* ── SP28: Side nav dots ── */
  .nav-dots { position: fixed; right: 16px; top: 50%; transform: translateY(-50%); display: flex; flex-direction: column; gap: 7px; z-index: 99; }
  .nav-dot { width: 8px; height: 8px; border-radius: 50%; background: rgba(0,0,0,0.2); cursor: pointer; transition: all 0.2s; border: 1.5px solid transparent; }
  .nav-dot.active, .nav-dot:hover { background: var(--sp-gold); border-color: var(--sp-gold); transform: scale(1.3); }

  /* ── SP28: Pages container ── */
  .pages-container { display: flex; flex-direction: column; align-items: center; gap: 24px; width: 100%; padding: 0 16px; }
  ...
  /* ── V3 Header ── */
  .header { padding: 40px 50px 20px; display: flex; justify-content: space-between; align-items: flex-end; border-bottom: 2px solid var(--sp-gold); }
  .header-left { display: flex; flex-direction: column; gap: 6px; }
  .header-subtitle { font-family: var(--font-sans); font-size: 10px; color: var(--sp-gold); font-weight: 700; text-transform: uppercase; letter-spacing: 2px; }
  .header-title { font-family: var(--font-serif); font-size: 26px; font-weight: 700; color: var(--sp-navy); letter-spacing: -0.5px; line-height: 1; }
  .header-meta { font-family: var(--font-mono); font-size: 9px; color: var(--sp-text-muted); text-align: right; line-height: 1.4; }

  /* ── V3 Footer ── */
  .footer { position: absolute; bottom: 0; left: 0; right: 0; display: flex; flex-direction: column; background: var(--white); }
  ...
  .lead { font-size: 12px; color: var(--sp-text-muted); line-height: 1.65; margin-bottom: 12px; }
  .sh   { font-size: 9.5px; font-weight: 700; letter-spacing: .18em; text-transform: uppercase; color: var(--sp-navy); margin: 14px 0 8px; display: flex; align-items: center; gap: 10px; }
  ...
  .bento-card { background: var(--white); border: 1px solid var(--border); border-radius: 2px; padding: 24px; display: flex; flex-direction: column; position: relative; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.02); }
  .bento-card::before { content: ''; position: absolute; top: 0; left: 0; width: 100%; height: 3px; background: var(--sp-gold); opacity: 0.3; }
  .bento-card.full-width { grid-column: 1 / -1; }
  .bento-card.highlight { background: var(--sp-navy); color: var(--white); border: none; }
  ...
  .bento-card.success { background: #FFFBEB; border: 1px solid var(--sp-gold-light); }
  ...
  .bento-label { font-family: var(--font-sans); font-size: 11px; font-weight: 700; color: var(--sp-text-muted); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 12px; }
  .bento-value { font-family: var(--font-mono); font-size: 28px; font-weight: 700; letter-spacing: -1px; margin-bottom: 6px; color: var(--sp-navy); }
  .bento-desc  { font-size: 11px; color: var(--sp-text-muted); line-height: 1.5; }

  /* ── V3 Data table ── */
  .data-table { width: 100%; border-collapse: collapse; font-size: 11px; margin-top: 20px; border-top: 2px solid var(--sp-navy); border-bottom: 2px solid var(--sp-navy); }
  .data-table th { text-align: left; padding: 12px 16px; background: var(--sp-gray); color: var(--sp-navy); font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; border-bottom: 1px solid var(--border); }
  .data-table td { padding: 12px 16px; border-bottom: 1px solid var(--border); color: var(--sp-text); font-family: var(--font-mono); }
  .data-table tr:last-child td { border-bottom: none; }
  .data-table .col-highlight { background: #FFFBEB; font-weight: 700; color: var(--sp-navy); }
  .data-table .row-total td  { background: var(--sp-navy); color: var(--white); font-weight: 700; border-top: 2px solid var(--sp-navy); }
  .data-table .row-total .col-highlight { background: #1e293b; color: var(--sp-gold); }

  /* ── V3 .dt table (with navy header) ── */
  ...
  .sub { display: block; font-size: 9px; color: var(--sp-text-muted); font-weight: 400; margin-top: 2px; font-family: var(--font-sans); }
  .dt .old { color: #DC2626; text-align: right; }
  .dt .new { font-weight: 600; color: var(--sp-navy); text-align: right; }
  .dt .save { color: var(--success); text-align: right; font-weight: 700; }
  .dt .cost { color: #DC2626; text-align: right; font-weight: 600; }
  ...
  .badge-navy, .badge-blue { background: var(--sp-navy); color: var(--white); }
  .badge-gold    { background: var(--sp-gold); color: var(--sp-navy); }
  ...
  /* ── V3 Cover (cover-v3-container) ── */
  .cover-v3-container { height: 100%; display: flex; position: relative; background: var(--white); }
  .cover-v3-content   { flex: 1; padding: 60px; display: flex; flex-direction: column; z-index: 2; }
  .cover-v3-sidebar   { width: 28%; background: var(--sp-navy); position: relative; overflow: hidden; }
  .cover-v3-sidebar::before {
    content: ''; position: absolute; top: 0; left: 0; right: 0; bottom: 0;
    background: repeating-linear-gradient(45deg, rgba(255,255,255,0.03), rgba(255,255,255,0.03) 1px, transparent 1px, transparent 10px);
  }
  .cover-v3-logo {
    font-size: 16px; font-weight: 800; letter-spacing: 3px; color: var(--sp-navy);
    text-transform: uppercase; margin-bottom: 100px; display: flex; align-items: center; gap: 10px;
  }
  .cover-v3-logo::before { content: ''; display: block; width: 24px; height: 24px; background: var(--sp-gold); }
  .cover-v3-badge {
    display: inline-block; padding: 6px 12px; background: #F1F5F9; color: var(--sp-navy);
    font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px;
    margin-bottom: 30px; border-left: 3px solid var(--sp-gold);
  }
  .cover-v3-title      { font-family: var(--font-serif); font-size: 68px; line-height: 0.95; color: var(--sp-navy); margin-bottom: 24px; letter-spacing: -2px; }
  .cover-v3-title span { color: var(--sp-gold); font-style: italic; }
  .cover-v3-subtitle   { font-family: var(--font-sans); font-size: 18px; line-height: 1.5; color: var(--sp-text-muted); max-width: 80%; margin-bottom: 80px; }
  .cover-v3-meta       { margin-top: auto; border-top: 1px solid var(--border); padding-top: 30px; display: grid; grid-template-columns: 1fr 1fr; gap: 40px; }
  .cv3-label { font-size: 10px; text-transform: uppercase; color: var(--sp-text-muted); letter-spacing: 1px; margin-bottom: 6px; }
  .cv3-value { font-size: 14px; font-weight: 600; color: var(--sp-navy); }
  .cv3-value.mono { font-family: var(--font-mono); font-size: 13px; }

  /* ── KPI grid ── */
  .kpi-grid { display: grid; grid-template-columns: repeat(3,1fr); gap: 14px; margin-bottom: 18px; }
  .kpi-box   { background: var(--sp-gray); border: 1px solid var(--border); border-top: 3px solid var(--sp-navy); padding: 14px 16px; }
  .kpi-box.highlight { background: var(--sp-navy); border-color: var(--sp-navy); border-top-color: var(--sp-gold); }
  .kpi-box.success   { border-top-color: var(--success); }
  .kpi-label { font-size: 8.5px; font-weight: 700; letter-spacing: .12em; text-transform: uppercase; color: var(--sp-text-muted); margin-bottom: 4px; }
  .kpi-box.highlight .kpi-label { color: rgba(255,255,255,.5); }
  .kpi-value { font-family: var(--font-mono); font-size: 20px; font-weight: 700; color: var(--sp-navy); line-height: 1.1; }
  .kpi-box.highlight .kpi-value { color: var(--sp-gold); }
  .kpi-box.success   .kpi-value { color: var(--success); }
  .kpi-sub { font-size: 9px; color: var(--sp-text-muted); margin-top: 3px; }
  .kpi-box.highlight .kpi-sub { color: rgba(255,255,255,.35); }

  /* ── V3 Roadmap ── */
  .roadmap-step { display: flex; gap: 20px; position: relative; padding-bottom: 30px; }
  .roadmap-step:last-child { padding-bottom: 0; }
  .roadmap-line { position: absolute; top: 35px; left: 16px; bottom: -5px; width: 2px; background: #E2E8F0; z-index: 0; }
  .roadmap-step:last-child .roadmap-line { display: none; }
  .roadmap-marker { width: 34px; height: 34px; border-radius: 50%; background: var(--white); border: 2px solid var(--sp-navy); color: var(--sp-navy); display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 14px; position: relative; z-index: 1; flex-shrink: 0; }
  .roadmap-marker.active { background: var(--sp-navy); color: var(--white); box-shadow: 0 0 0 4px #E0E7FF; }
  .roadmap-marker.final  { background: var(--sp-gold); border-color: var(--sp-gold); color: var(--white); box-shadow: 0 0 0 4px #FEF3C7; }
  .roadmap-content { padding-top: 4px; }
  .roadmap-title { font-size: 14px; font-weight: 700; color: var(--sp-navy); text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 6px; }
  .roadmap-desc  { font-size: 12px; color: var(--sp-text-muted); line-height: 1.5; }
  ...
  /* ── Legacy proc-step (step list w/ large number) ── */
  .proc-krok  { font-family: var(--font-mono); font-size: 26px; font-weight: 700; color: var(--sp-gold); flex-shrink: 0; width: 40px; line-height: 1; }
  .proc-title { font-size: 12px; font-weight: 700; color: var(--sp-navy); margin-bottom: 4px; text-transform: uppercase; letter-spacing: 0.3px; }
  .proc-badge { display: inline-block; font-size: 7.5px; font-weight: 700; letter-spacing: .1em; padding: 2px 7px; background: var(--sp-gold); color: var(--sp-navy); margin-left: 8px; vertical-align: middle; text-transform: uppercase; }
  ...
  .box-green { border: 1px solid var(--success); border-left: 4px solid var(--success); background: #ECFDF5; padding: 12px 16px; margin-bottom: 10px; }
  ...
  /* ── Legal pillars ── */
  .pillar      { display: flex; gap: 14px; padding: 10px 0; border-bottom: 1px solid var(--border); }
  .pillar:last-child { border-bottom: none; }
  .pillar-badge     { font-family: var(--font-mono); font-size: 18px; font-weight: 700; color: var(--sp-gold); flex-shrink: 0; width: 26px; line-height: 1.3; }
  .pillar-badge-lbl { font-size: 8px; font-weight: 700; letter-spacing: .12em; text-transform: uppercase; color: var(--sp-text-muted); margin-bottom: 2px; }
  .pillar-title     { font-size: 11px; font-weight: 700; color: var(--sp-navy); margin-bottom: 3px; }
  .pillar-ref       { font-size: 8px; color: var(--sp-text-muted); font-style: italic; margin-top: 2px; }

  /* ── Model split ── */
  .model-split               { display: grid; grid-template-columns: 1fr 1fr; border: 1px solid var(--border); border-top: 2px solid var(--sp-navy); overflow: hidden; margin-bottom: 18px; }
  .model-part                { padding: 18px 22px; }
  .model-part:first-child    { background: var(--sp-gray); border-right: 1px solid var(--border); }
  .model-part:last-child     { background: #ECFDF5; }
  .model-part-lbl            { font-size: 9px; font-weight: 700; letter-spacing: .12em; text-transform: uppercase; color: var(--sp-text-muted); margin-bottom: 4px; }
  .model-part-amount         { font-family: var(--font-mono); font-size: 30px; font-weight: 700; color: var(--sp-navy); margin-bottom: 3px; line-height: 1; }
  .model-part:last-child .model-part-amount { color: var(--success); }
  .model-part-desc           { font-size: 9.5px; color: var(--sp-text-muted); line-height: 1.5; }
  ...
  .vt thead th.h-std  { background: var(--sp-navy); color: var(--white); }
  .vt thead th.h-plus { background: var(--sp-gold); color: var(--sp-navy); }
  .vt thead th.h-feature { text-align: left; background: var(--sp-gray); border: 1px solid var(--border); font-size: 9px; text-transform: uppercase; letter-spacing: .1em; color: var(--sp-text-muted); }
  .vt tbody tr { border-bottom: 1px solid var(--border); }
  .vt tbody tr:nth-child(even) td { background: var(--sp-gray); }
  ...
  .vt td.chk { text-align: center; color: var(--success); font-weight: 700; font-size: 14px; }
  .vt td.no  { text-align: center; color: #9CA3AF; font-size: 14px; }
  .vt td.tc  { text-align: center; font-weight: 600; color: var(--sp-navy); font-family: var(--font-mono); }
  .yes { color: var(--success); font-weight: 700; font-size: 14px; }
  .no  { color: var(--sp-text-muted); font-size: 14px; }

  /* ── Platform features ── */
  .feat-grid    { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 14px; margin-bottom: 14px; }
  .feat-col     { background: var(--sp-gray); border: 1px solid var(--border); border-top: 3px solid var(--sp-navy); padding: 12px 14px; }
  .feat-col-lbl { font-size: 9px; font-weight: 700; letter-spacing: .12em; text-transform: uppercase; color: var(--sp-navy); margin-bottom: 9px; padding-bottom: 7px; border-bottom: 1px solid var(--border); }
  .feat-item    { font-size: 10px; color: var(--sp-text-muted); padding: 5px 0; border-bottom: 1px solid var(--border); line-height: 1.4; }
  .feat-item:last-child { border-bottom: none; }
  .feat-item::before { content: '—'; color: var(--sp-gold); margin-right: 6px; font-weight: 700; }

  /* ── Promise boxes ── */
  .promise-grid  { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 12px; margin-bottom: 14px; }
  .promise-box   { background: var(--sp-navy); border-top: 3px solid var(--sp-gold); padding: 14px 16px; }
  .promise-label { font-size: 8.5px; font-weight: 700; letter-spacing: .14em; text-transform: uppercase; color: var(--sp-gold); margin-bottom: 5px; }
  .promise-text  { font-size: 10.5px; color: rgba(255,255,255,.8); line-height: 1.55; }

  /* ── Start columns ── */
  .start-grid    { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 14px; margin-bottom: 18px; }
  .start-col     { text-align: center; background: var(--sp-gray); border: 1px solid var(--border); border-top: 3px solid var(--sp-navy); padding: 22px 16px 18px; }
  .start-circnum { font-family: var(--font-mono); font-size: 30px; font-weight: 700; color: var(--sp-gold); line-height: 1; margin-bottom: 9px; }
  .start-title   { font-size: 12px; font-weight: 700; color: var(--sp-navy); margin-bottom: 5px; }
  .start-desc    { font-size: 10px; color: var(--sp-text-muted); line-height: 1.55; }
  .start-tag     { display: inline-block; margin-top: 10px; font-size: 7.5px; font-weight: 700; letter-spacing: .1em; text-transform: uppercase; padding: 3px 9px; background: var(--sp-navy); color: var(--sp-gold); }

  /* ── Nota sections ── */
  .nota-s     { margin-bottom: 8px; padding: 8px 0; border-bottom: 1px solid var(--border); }
  .nota-s:last-child { border-bottom: none; }
  .nota-title { font-size: 9px; font-weight: 700; text-transform: uppercase; letter-spacing: .1em; color: var(--sp-navy); margin-bottom: 3px; }
  .nota-text  { font-size: 8.5px; color: var(--sp-text-muted); line-height: 1.55; }

  /* ── Step boxes ── */
  .step-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
  .step-box  { background: rgba(255,255,255,.04); border: 1px solid rgba(255,255,255,.09); padding: 13px 15px; display: flex; gap: 12px; align-items: flex-start; }
  .step-num  { width: 24px; height: 24px; border-radius: 50%; background: var(--sp-gold); color: var(--sp-navy); font-size: 11px; font-weight: 700; display: flex; align-items: center; justify-content: center; flex-shrink: 0; font-family: var(--font-mono); }
  ...
  /* ── Print ── */
  @media print {
    .toolbar, .nav-dots { display: none !important; }
    body { background: none !important; padding: 0 !important; display: block !important; }
    .pages-container { display: block !important; padding: 0 !important; }
    .page { box-shadow: none; margin: 0; page-break-after: always; break-after: page; width: 210mm; height: 297mm; overflow: hidden; }
