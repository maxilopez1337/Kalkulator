...
export const offerPdfV3Generator = {
  generateOfferPDF: (item: ZapisanaKalkulacja, sector: string = 'MSP') => {
    ...
    const sumaKosztCurrent = details.reduce(
      (acc: number, w: any) => acc + w.currentModel.kosztPracodawcy,
      0
    );
    const sumaKosztElitonBase = details.reduce(
      (acc: number, w: any) => acc + w.elitonBase.kosztPracodawcy,
      0
    );
    ...
    const sumaZusPracodawcy = details.reduce(
      (acc: number, w: any) => acc + (w.currentModel.kosztPracodawcy - w.currentModel.brutto),
      0
    );
    const sumaBrutto = details.reduce((acc: number, w: any) => acc + w.currentModel.brutto, 0);

    const sumaZasadnicza = details.reduce(
      (acc: number, w: any) => acc + w.elitonBase.zasadnicza.brutto,
      0
    );
    const sumaSwiadczenieBrutto = details.reduce(
      (acc: number, w: any) => acc + w.elitonBase.swiadczenie.brutto,
      0
    );
    ...
    const sumaZusPracownika = details.reduce(
      (acc: number, w: any) => acc + w.currentModel.zusPracownik.suma + w.currentModel.zdrowotna,
      0
    );
    const sumaZusPracownikaEliton = details.reduce(
      (acc: number, w: any) =>
        acc + w.elitonBase.zasadnicza.zusPracownik.suma + w.elitonBase.zasadnicza.zdrowotna,
      0
    );
    ...
    const sumaNettoZasadnicze = details.reduce(
      (acc: number, w: any) => acc + (w.elitonBase.zasadnicza.nettoGotowka || 0),
      0
    );
