...
const normalizeDate = (val: any): string => {
    ...
    if (str.match(/^\d{2}-\d{2}-\d{4}/)) { // Format DD-MM-YYYY
        const parts = str.split('-');
...
export const parseExcelData = (rows: any[], config: Config): ImportRow[] => {
    ...
    return dataRows.map((row, idx) => {
        ...
        if (
            rawZus.includes('BEZ DOBROWOLNEJ CHOROBOWEJ') || 
            rawZus === 'BEZ CHOROBOWEJ'
        ) {
            ...
            rawZus.includes('INNY TYTUŁ') || 
            rawZus.includes('TYLKO ZDROWOTNA')
        ) {
            trybSkladek = 'INNY_TYTUL';
            choroboweAktywne = false;
        } 
        else if (
            rawZus.includes('EMERYT') || 
            rawZus.includes('RENCISTA')
        ) {
            ...
            rawZus.includes('PEŁNE SKŁADKI') || 
            rawZus.includes('PELNE SKLADKI')
        ) {
        ...
        const finalData: Pracownik = {
            id: Date.now() + idx,
            imie, nazwisko, dataUrodzenia, plec, typUmowy, trybSkladek, choroboweAktywne,
            pit2, ulgaMlodych, kupTyp, nettoDocelowe: netto,
            nettoZasadnicza: typUmowy === 'UZ' ? config.minimalnaKwotaUZ.zasadniczaNetto : config.placaMinimalna.netto,
            pitMode, skladkaFP, skladkaFGSP
        };
