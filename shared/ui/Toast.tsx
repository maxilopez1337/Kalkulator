...
export const Toast = ({ notification, onClose }: ToastProps) => {
    ...
    return (
        <div className={`fixed bottom-6 right-6 z-[100] max-w-sm w-full bg-white rounded-md shadow-2xl border-l-4 overflow-hidden animate-in slide-in-from-bottom-5 fade-in duration-300 flex items-center p-4 gap-3 ${isSuccess ? 'border-emerald-500' : isError ? 'border-rose-500' : 'border-blue-500'}`}>
            ...
            <div className="flex-1">
                ...
                <p className="text-xs text-[#605e5c] mt-0.5">{notification.message}</p>
            </div>
            <button onClick={onClose} className="text-[#a19f9d] hover:text-[#605e5c] p-1">
                <X className="w-4 h-4" />
            </button>
        </div>
    );
