...
import { TrendingUp, Save, FileText, Mail, ShieldCheck, PieChart, Calculator } from '../../../shared/icons/Icons';
...
export const StepPodsumowanie = ({ onGoToDashboard }: StepPodsumowanieProps) => {
  const { wyniki, prowizjaProc, setProwizjaProc, setComparisonState, saveToHistory, firma, generateOfferElitonPrimePlus, generateLegalizacjaPremii, pracownicy, downloadCalculation, config, comparisonState } = useAppStore();
  ...
  const {
      ...
      chartData,
  } = useSummaryData({ szczegoly: wyniki?.szczegoly, prowizjaProc, activeModel, selectedPracownikId });
  ...
  const handleToggleModel = () => {
      if (activeModel === 'PRIME') {
          setComparisonState((prev: any) => ({ ...prev, activeCard: 'STANDARD' }));
          setProwizjaProc(comparisonState.customStandardRate);
      } else {
          setComparisonState((prev: any) => ({ ...prev, activeCard: 'PRIME' }));
    ...
    <div className={animations.fadeIn}>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-4 md:gap-6 items-start">
            ...
            <div className="xl:col-span-8 flex flex-col gap-6">
                ...
                <div className={`bg-white rounded-md border border-[#edebe9] ${shadow.elevation4} overflow-hidden`}>
                    <div className="px-5 py-3 flex items-center gap-2.5 border-b border-[#00245b] bg-brand rounded-t-md">
                        ...
                        <span className="ml-auto text-[10px] text-[#a19f9d] hidden sm:block">Eliton Prime™ — alokacja kosztów</span>
                    </div>
                    <DonutChart
                        data={chartData}
