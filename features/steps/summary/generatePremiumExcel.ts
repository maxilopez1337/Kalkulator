...
export const generatePremiumExcel = async ({ firma, wyniki, prowizjaProc }: GeneratorOptions) => {
  ...
  wsDash.getCell('B2').value =
    `Ilustracja finansowa oszczędności po wdrożeniu modelu Eliton Prime™ PLUS dla firmy : ${firma.nazwa || 'FIRMA'}`;
  ...
  const addDashboardRow = (
    ...
    trgValueOrFormula: number | { formula: string },
    r: number,
    isTotal = false,
    customStyle: { font?: Record<string, unknown>; fill?: Record<string, unknown> } = {}
  ) => {
  ...
  const statsStratton = {
    ...
    zus: wyniki.szczegoly.reduce((acc, w) => {
      ...
      return (
        acc +
        round(isStudent ? w.standard.zusPracodawca.suma : w.podzial.zasadnicza.zusPracodawca.suma)
      );
    }, 0),
    prowizjaTotal: round(
  ...
  if (isPlus) {
    ...
    addDashboardRow(
      ...
      0,
      { formula: "'Kalkulator Podwyżek'!$P$6" },
      currentRow++,
      false,
      {
    ...
    addDashboardRow(
      'Bonus dla administracji w (Stratton 2%)',
      0,
      { formula: "'Kalkulator Podwyżek'!$P$11" },
      currentRow++,
      false,
      {
  ...
  addDashboardRow(
    'Budżet na dodatkowe podwyżki od pracodawcy',
    0,
    { formula: "'Kalkulator Podwyżek'!$P$7" },
    currentRow++,
    false,
    {
  ...
  wsDash.getCell(`C${totalRowIdx}`).value = {
    formula: `SUM(C${tableRow + 1}:C${totalRowIdx - 1})`,
  };
  wsDash.getCell(`D${totalRowIdx}`).value = {
    formula: `SUM(D${tableRow + 1}:D${totalRowIdx - 1})`,
  };
  ...
  // Input użytkownika
  wsSim.getCell('G2').value =
    'Dodatkowa podwyżka od pracodawcy dla wszystkich pracowników (% od Obence Netto):';
  ...
  wyniki.szczegoly.forEach((w, i) => {
    ...
    if (isStudent) {
      ...
      for (let cCode = 65; cCode <= 77; cCode++) {
        // A-M
        wsSim.getCell(`${String.fromCharCode(cCode)}${r}`).fill = {
          type: 'pattern',
          pattern: 'solid',
  ...
  for (let c = 3; c <= 13; c++) {
    ...
    if (c <= 11 || c === 13) {
      ...
      wsSim.getCell(`${colLetter}${sumRow}`).value = {
  ...
  wsSim.getCell(`${summaryStartCol}${sumStartRow + 1}`).value =
    '+4% świadczeń rzeczowych EBS finansowane przez Stratton Prime';
  wsSim.getCell(`${summaryStartCol}${sumStartRow + 1}`).font = {
    size: 10,
    color: { argb: 'FF059669' },
    bold: true,
  };
  wsSim.getCell(`${summaryValueCol}${sumStartRow + 1}`).value = {
  ...
  wsSim.getCell(`${summaryStartCol}${sumStartRow + 2}`).value =
    'Dodatkowa Podwyżka (od Pracodawca)';
  wsSim.getCell(`${summaryStartCol}${sumStartRow + 2}`).font = {
    size: 10,
    bold: true,
    color: { argb: 'FFD97706' },
  };
  wsSim.getCell(`${summaryValueCol}${sumStartRow + 2}`).value = {
  ...
  wsSim.getCell(`${summaryValueCol}${sumStartRow + 2}`).fill = {
    type: 'pattern',
    pattern: 'solid',
  ...
  wsSim.getCell(`${summaryValueCol}${sumStartRow + 3}`).value = {
    formula: `SUM(${summaryValueCol}${sumStartRow + 1}:${summaryValueCol}${sumStartRow + 2})`,
  };
  wsSim.getCell(`${summaryValueCol}${sumStartRow + 3}`).numFmt = styles.currency;
  wsSim.getCell(`${summaryValueCol}${sumStartRow + 3}`).font = {
    bold: true,
    size: 12,
    color: { argb: 'FF059669' },
  };
