interface WynikParam {
  standard: {
    netto: number;
    brutto: number;
    pit: number;
    kosztPracodawcy: number;
    zusPracownik?: { suma: number };
  };
  podzial: {
    zasadnicza: { brutto: number; netto: number; kosztPracodawcy: number };
    swiadczenie?: { netto: number; brutto: number };
  };
}

interface SymulacjaParam {
  noweBruttolaczne: number;
  zaliczkaPit: number;
  wynagrodzenieDoWyplaty: number;
  nowyKosztPracodawcy: number;
}
...
export const generateEmployeeSimulationExcel = async (
  ...
  // Arkusz 3: Eliton Prime™
  const wsEliton = workbook.addWorksheet('3. Eliton Prime™');
  wsEliton.columns = [
    { header: 'Parametr', key: 'param', width: 30 },
    { header: 'Wartość', key: 'wartosc', width: 25, style: { numFmt: currencyFmt } },
  ];
  const swiadczenieWynik = wynik.podzial.swiadczenie?.netto || 0;
  ...
  wsEliton.addRow({
    param: 'Łączny koszt brutto (z ZUS pracodawcy)',
    wartosc: wynik.podzial.zasadnicza?.kosztPracodawcy || 0,
  });
  wsEliton.addRow({
    param: 'Wynagrodzenie netto łącznie',
    wartosc: wynik.podzial.zasadnicza.netto + swiadczenieWynik,
  });
  ...
  wsPodsumowanie.columns = [
    ...
    {
      header: 'Model symulowany rocznie',
      key: 'eliton',
      width: 25,
      style: { numFmt: currencyFmt },
    },
