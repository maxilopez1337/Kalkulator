...
export const StepAnalizaPracownika = () => {
  ...
  const pracownicy: WynikPracownika[] = wyniki?.szczegoly || [];

  // ─── State helpers
  const setRaise = (id: string | number, field: 'ebs' | 'employer', val: string) =>
    setRaises((prev) => ({
      ...prev,
      [id]: { ...(prev[id] ?? { ebs: '', employer: '' }), [field]: val },
    }));

  const applyGlobal = () => {
    const next: Record<string | number, RaiseEntry> = {};
    pracownicy.forEach((p: WynikPracownika) => {
      next[p.pracownik.id] = { ebs: globalEBS, employer: globalEmployer };
    });
    setRaises(next);
  };
  ...
  const tableData = useMemo(
    () =>
      pracownicy.map((w: WynikPracownika) => {
        ...
        const findSBrutto = (
          targetNetto: number,
          narastBefore: number,
          ulgaAvailable: number
        ): { s_brutto: number; pit: number; dochod: number } => {
        ...
        let narastStd = 0,
          narastEli = 0;
        let stdIIProg: number | null = null,
          eliIIProg: number | null = null;
        let ulgaUsedStd = 0,
          ulgaUsedEli = 0; // obie zmienne używane w pętli

        for (let m = 0; m < 12; m++) {
          // Standard month
          {
            const dochoBase = Math.max(0, std_brutto - std_zus - std_kup);
            ...
            stdMonths.push({
              ...
              netto,
              dochod: taxable,
              narastajace: narastStd,
              isIIProg: narastStd >= p1Limit,
            });
          }
          ...
          if (isStudent) {
            eliMonths.push({
              brutto: z_brutto + totalSwNetto,
              s_brutto: totalSwNetto,
              pit: 0,
              netto: eliTargetNetto,
              dochod: 0,
              narastajace: narastEli,
              isIIProg: false,
            });
          } else {
            ...
            eliMonths.push({
              brutto: z_brutto + s_brutto,
              s_brutto,
              pit,
              netto,
              dochod,
              narastajace: narastEli,
              isIIProg: narastEli >= p1Limit,
            });
          }
        }
        ...
        <div className="px-4 md:px-6 py-3 flex flex-col md:flex-row gap-3 md:items-center md:justify-between">
          <div>
            <div className="flex items-center gap-2 mb-0.5">
              ...
              <h1 className="text-sm md:text-base font-bold text-[#201f1e] leading-tight">
                Analiza PIT-37 – Symulacja Podatku Pracownika
              </h1>
            </div>
            <p className="hidden sm:block text-[11px] text-[#605e5c] pl-3.5">
              Model Tradycyjny vs. Eliton Prime™ &nbsp;·&nbsp; Skala 12% / 32% (narastająco)
              &nbsp;·&nbsp; Miesiąc po miesiącu &nbsp;·&nbsp; Próg II: 120 000 zł/rok
            </p>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap shrink-0">
            <div className="flex items-center gap-1.5 bg-[#eff6fc] border border-[#c8e1f7] rounded px-3 py-1.5">
              <span className="text-[10px] text-[#0078d4] font-bold uppercase tracking-wider whitespace-nowrap">
                Podwyżka EBS netto:
              </span>
              <input
                type="number"
                value={globalEBS}
                onChange={(e) => setGlobalEBS(e.target.value)}
                placeholder="0 zł"
            ...
            <div className="flex items-center gap-1.5 bg-emerald-50 border border-emerald-200 rounded px-3 py-1.5">
              <span className="text-[10px] text-emerald-700 font-bold uppercase tracking-wider whitespace-nowrap">
                Podwyżka Prac. netto:
              </span>
              <input
                type="number"
                value={globalEmployer}
                onChange={(e) => setGlobalEmployer(e.target.value)}
                placeholder="0 zł"
