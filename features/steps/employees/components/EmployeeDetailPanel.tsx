...
import { ConfirmOptions } from '../../../../store/ConfirmContext';
...
const SectionHeader = ({ label }: { label: string }) => (
  <div className="flex items-center gap-2 mb-3">
    <span className="text-[10px] font-bold text-[#0078d4] uppercase tracking-widest whitespace-nowrap">
      {label}
    </span>
...
interface EmployeeDetailPanelProps {
  ...
  activeEmployee: Pracownik | null;
  onDeselect: () => void;
  onDuplicate: (id: number) => void;
  onRemove: (id: number) => void;
  onUpdate: (id: number, field: keyof Pracownik, value: unknown) => void;
  onConfirm: (message: string, options?: ConfirmOptions) => Promise<boolean>;
}
  ...
  >
    {activeEmployee === null ? (
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center p-8 select-none">
        ...
        <p className="text-[13px] text-[#a19f9d] max-w-xs">
          Kliknij pracownika na liście aby wyświetlić i edytować jego dane.
        </p>
      </div>
    ) : (
      <div className="flex flex-col h-full overflow-hidden animate-in fade-in duration-150">
        ...
        <div className="shrink-0 bg-white border-b border-[#edebe9] px-3 md:px-6 py-3 flex items-center justify-between gap-2 sm:gap-4">
          <div className="flex items-center gap-2 md:gap-3 min-w-0">
            ...
            <Avatar
              name={activeEmployee.imie}
              surname={activeEmployee.nazwisko}
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-2 mt-0.5">
                ...
                <span className="text-[12px] text-[#605e5c] font-mono">
                  {formatPLN(activeEmployee.nettoDocelowe)}
                </span>
                {activeEmployee.trybSkladek !== 'PELNE' && (
                  <span className="text-[10px] bg-[#fff4ce] text-[#d47500] px-1.5 py-0.5 rounded-sm border border-[#fde5c4]">
                    Specjalny tryb ZUS
                  </span>
                )}
              </div>
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0">
            <ButtonSecondary
              size="sm"
              onClick={() => onDuplicate(activeEmployee.id)}
              icon={<Copy />}
              title="Duplikuj"
            >
            ...
            <ButtonDanger
              ...
              onClick={async () => {
                if (
                  await onConfirm(pl.confirms.deleteSelectedEmployees(1), { variant: 'danger' })
                ) {
                  onRemove(activeEmployee.id);
                  onDeselect();
                }
              }}
            >
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-5">
          ...
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-x-5 gap-y-3 mb-5">
            <FormField label="Imię">
              <Input
                value={activeEmployee.imie}
                onChange={(e) => onUpdate(activeEmployee.id, 'imie', e.target.value)}
              />
            </FormField>
            <FormField label="Nazwisko">
              <Input
                value={activeEmployee.nazwisko}
                onChange={(e) => onUpdate(activeEmployee.id, 'nazwisko', e.target.value)}
              />
            </FormField>
            <FormField label="Data urodzenia">
              <Input
                type="date"
                value={activeEmployee.dataUrodzenia}
                onChange={(e) => onUpdate(activeEmployee.id, 'dataUrodzenia', e.target.value)}
              />
            </FormField>
            <FormField label="Płeć">
              <Select
                value={activeEmployee.plec}
                onChange={(e) => onUpdate(activeEmployee.id, 'plec', e.target.value)}
              >
          ...
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-x-5 gap-y-3 mb-5">
            <FormField label="Rodzaj umowy">
              <Select
                value={activeEmployee.typUmowy}
                onChange={(e) => onUpdate(activeEmployee.id, 'typUmowy', e.target.value)}
              >
            ...
            <FormField label="Wynagrodzenie Netto">
              <NettoInput
                ...
                onCommit={(v) => onUpdate(activeEmployee.id, 'nettoDocelowe', v)}
            ...
            <FormField label="Tryb ZUS">
              <Select
                value={activeEmployee.trybSkladek}
                onChange={(e) => onUpdate(activeEmployee.id, 'trybSkladek', e.target.value)}
              >
            ...
            <FormField label="Koszty Uzyskania (KUP)">
              <Select
                value={activeEmployee.kupTyp}
                onChange={(e) => onUpdate(activeEmployee.id, 'kupTyp', e.target.value)}
              >
          ...
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-x-5 gap-y-3">
            <FormField label="Kwota wolna (PIT-2)">
              <Select
                value={activeEmployee.pit2}
                onChange={(e) => onUpdate(activeEmployee.id, 'pit2', e.target.value)}
              >
            ...
            <FormField label="Zaliczka PIT">
              <Select
                value={activeEmployee.pitMode || 'AUTO'}
                onChange={(e) => onUpdate(activeEmployee.id, 'pitMode', e.target.value)}
              >
            ...
            <div className="flex flex-col gap-3 pt-5">
              <label className="flex items-center gap-2.5 cursor-pointer select-none group/cb">
                <div
                  className={`w-4 h-4 rounded-sm border flex items-center justify-center flex-shrink-0 transition-colors text-white ${activeEmployee.ulgaMlodych ? 'bg-[#0078d4] border-[#0078d4]' : 'bg-white border-[#8a8886] group-hover/cb:border-[#0078d4]'}`}
                >
                ...
                <input
                  type="checkbox"
                  className="hidden"
                  checked={activeEmployee.ulgaMlodych}
                  onChange={(e) => onUpdate(activeEmployee.id, 'ulgaMlodych', e.target.checked)}
                />
                <span className="text-[13px] text-[#323130]">Ulga dla młodych (&lt;26)</span>
              </label>
              <label className="flex items-center gap-2.5 cursor-pointer select-none group/cb">
                <div
                  className={`w-4 h-4 rounded-sm border flex items-center justify-center flex-shrink-0 transition-colors text-white ${!activeEmployee.skladkaFP ? 'bg-[#107c10] border-[#107c10]' : 'bg-white border-[#8a8886] group-hover/cb:border-[#107c10]'}`}
                >
                ...
                <input
                  type="checkbox"
                  className="hidden"
                  checked={!activeEmployee.skladkaFP}
                  onChange={(e) => onUpdate(activeEmployee.id, 'skladkaFP', !e.target.checked)}
                />
